...
function displayMatchReady(params) {
    const opponent = params.match.agents?.find((a) => a.id !== params.myAgentId);
    console.log();
...
function normalizeMatchesResponse(response) {
...
export function arenaCommand(program) {
    const arena = program
        .command('arena')
    ...
    arena
        ...
        // Validate arena
        const selectedArena = ARENAS.find(a => a.id === arenaName.toLowerCase() || a.name.toLowerCase() === arenaName.toLowerCase());
        if (!selectedArena) {
    ...
    arena
        ...
        try {
            ...
            console.log(table(['Arena', 'Match', 'Prize', 'Round', 'Viewers', 'ID'], rows));
            console.log();
