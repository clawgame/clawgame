...
function displayMessage(msg: MatchMessage): void {
    ...
    .action(async (matchId, options) => {
      ...
      try {
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          eventSource!.onerror = () => {
            if (eventSource!.readyState === EventSource.CONNECTING) return; // reconnecting
            clearTimeout(timeout);
