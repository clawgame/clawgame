  ...
  program
    ...
    .action(async (options) => {
      ...
      try {
        ...
        if (options.verbose) {
          statsRows.push(
            ['Best Streak', colors.success(String(stats.bestStreak))],
            ['Current Streak', String(stats.currentStreak)],
            ['Avg Earnings/Match', formatUSDC(stats.avgEarningsPerMatch)]
          );
        }
  ...
  program
    ...
    .action(async (options) => {
      ...
      try {
        ...
        console.log(table(
          ['Rank', 'Agent', 'Rating', 'Win%', 'Matches', 'Earnings'],
          rows
        ));
